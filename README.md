...
| 3 | GitHub Actions | EAS Cloud Build **または** Fastlane Match ビルド | [docs/03-github-actions-eas-fastlane.md](./docs/03-github-actions-eas-fastlane.md) |
| 4 | Xcode Cloud Build | Apple 公式 CI（iOS のみ） | [docs/04-xcode-cloud.md](./docs/04-xcode-cloud.md) |

必要な Secrets / 環境変数の一覧は [docs/secrets-and-env.md](./docs/secrets-and-env.md) にまとめています。
...
2. 以下を自分のプロジェクトの値に置き換える。
   ...
   | `package.json` | `name` |
   | `eas.json` | `submit.production` の Apple ID / ASC App ID / Team ID |
   | `fastlane/Matchfile` | 証明書用リポジトリの URL・種別（環境変数経由でも可） |

3. 使いたいビルド方法のドキュメント（上表）を読み、必要な Secrets / 環境変数を設定する。
...
├── App.tsx
├── docs/                    # 4つのビルド方法の詳細ガイド + secrets一覧
├── fastlane/                # 証明書管理(match) + iOS/Androidビルドレーン
├── .github/workflows/
...
| 観点 | Local EAS Build | EAS Cloud Build | GitHub Actions | Xcode Cloud Build |
|---|---|---|---|---|
| Mac 必須か | iOS は必須 | 不要 | 3-A: 不要 / 3-B: macOS ランナー | 不要（Apple 提供） |
| 証明書管理 | 手元 / match | EAS 管理 or 持ち込み | 3-A: EAS / 3-B: 専用 Git リポ | Apple 自動管理が基本 |
| セットアップの手間 | 小 | 小 | 中〜大 | 中 |
| 費用 | 無料（自前マシン） | 無料枠 + 従量課金 | GitHub Actions の分数課金 | Apple Developer Program に含まれる枠あり |
| Android 対応 | ○ | ○ | ○ | × |
| 主な用途 | ローカル検証・デバッグ | 汎用・チーム開発 | CI 自動化（EAS または Fastlane） | iOS 特化・Apple 完結志向 |
